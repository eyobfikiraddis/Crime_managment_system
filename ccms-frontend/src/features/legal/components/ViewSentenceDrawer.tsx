...
import { EditSentenceDrawer } from './EditSentenceDrawer'
...
export function ViewSentenceDrawer({
  ...
  const [editSentenceOpen, setEditSentenceOpen] = useState(false)
  ...
  return (
    <>
        ...
        footer={
          <div className="flex items-center justify-end gap-2 border-t border-border pt-4 w-full">
            ...
            <PermissionGuard permission={Permission.ADMIN_MANAGE}>
              <Button
                type="button"
                onClick={() => setEditSentenceOpen(true)}
                disabled={isLoading || isError || !sentence}
              >
                {t('charges.viewSentence.amendButton')}
              </Button>
            </PermissionGuard>
          </div>
        }
      ...
      {editSentenceOpen ? (
        <EditSentenceDrawer
          open={editSentenceOpen}
          onOpenChange={setEditSentenceOpen}
          chargeId={chargeId}
          courtCaseId={charge.courtCaseId}
